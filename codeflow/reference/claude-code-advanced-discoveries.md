...
#### Memory MCP (汎用メモリシステム) 🧠

```typescript
// メモリシステムがあれば、以下のような操作が可能
addMemory({ content: string })
search({ query: string })
```

**注意**: プロジェクトスコープはヘッダーや環境変数で制御
...
for (const issue of issues) {
  ...
  // 4. コミット
  await Bash(
    `git add . && git commit -m "${issue.key}: ${issue.fields.summary}"`
  )
...
| Memory System   | APIキー  | プロジェクトスコープ設定   |
...
**ファイル**: `.claude/skills/codeflow/reference/hearing-first.md`
