...
Code Flow の実装コアは、以下の 4 ステップで構成される（名前で参照、番号は使わない）：

```
🎯 ヒアリング → 📝 SDG → 💻 実装 → 🔄 Living Documentation
```

各ステップは明確な役割を持ち、シームレスに次のステップへ移行する。
...
pub enum SearchError { #[error("Empty query")]
EmptyQuery,
...
2. 正しい方に合わせて修正
3. 変更履歴に記録
4. Living Documentation ステップを再実行
```

## まとめ

Code Flow の開発フローは、以下の 4 ステップで構成される（名前で参照、番号は使わない）：

- **🎯 Hearing**: 要件を明確化
- **📝 SDG**: 仕様・設計を記録
- **💻 Implementation**: コードを実装
- **🔄 Living Documentation**: 同期を維持

各ステップは明確な役割を持ち、前のステップの成果物を次のステップで活用する。
このフローに従うことで、「ユーザーの期待に沿った実装」と「生きたドキュメント」の両方を実現できる。
